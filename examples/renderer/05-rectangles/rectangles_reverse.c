...
SDL_AppResult SDL_AppIterate(void *appstate)
{
    ...
    /* Red Rectangle is in reverse order thanks to change in scale*/
    rects[0].x = rects[0].y = 100 + (-100 * scale);
    ...
    SDL_RenderRects(renderer, rects, 4);  /* draw three rectangles at once */

    /* those were rectangle _outlines_, really. You can also draw _filled_ rectangles! */
    rects[0].x = 200 + (-100 * scale);
    rects[0].y = 200 + (-50 * scale);
